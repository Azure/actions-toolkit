...
import {
  IExecSyncResult,
  IExecSyncOptions
} from "@azure-actions/utilities/lib/toolrunner";
...
export async function getAzureAccessToken(creds?: string): Promise<string> {
  if (creds) {
    ...
    try {
      const token = getTokenUsingAzCLI();
...
async function getTokenUsingCredObject(creds: string) {
  ...
  const servicePrincipalId: string = credsObject["clientId"];
  const servicePrincipalKey: string = credsObject["clientSecret"];
  const tenantId: string = credsObject["tenantId"];
  const authorityUrl: string =
    credsObject["activeDirectoryEndpointUrl"] ||
    "https://login.microsoftonline.com";
  const managementEndpointUrl: string =
    credsObject["resourceManagerEndpointUrl"] ||
  ...
  if (
    ...
    throw new Error(
      "Not all values are present in the creds object. Ensure clientId, clientSecret, tenantId and activeDirectoryEndpointUrl are supplied in the provided object"
    );
  }
  return new Promise<string>((resolve, reject) => {
    ...
    sendRequest(webRequest, webRequestOptions).then(
      (response: WebResponse) => {
        ...
        } else if ([400, 401, 403].includes(response.statusCode)) {
...
function getTokenUsingAzCLI() {
  const resultOfExec: IExecSyncResult = execSync(
    "az",
    'account get-access-token --query "accessToken"',
    { silent: true } as IExecSyncOptions
  );
  if (resultOfExec.code != 0) {
    core.error(
      `Login with azure/login action to fetch token. Error Code: ${resultOfExec.code}`
    );
    throw resultOfExec;
  }
  const token = resultOfExec.stdout.trim();
